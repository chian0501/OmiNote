...
- `card-workspace-v1.js` 與 `card-workspace-v1.css` 整理編輯器介面；`card-direct-edit-v1.js` 追蹤原生文字座標並操作既有欄位。Canvas 正式樣式、輸出像素尺寸、JSON schema、本機暫存鍵值與原生事件處理保持相容。
...
GitHub Actions 的 `Card workspace browser QA` 使用鎖定版本 Chromium／Playwright，共 66 項檢查：12 個工具 × 1920／1366／390 px 的 36 組版面與鍵盤測試；12 組編輯、JSON 往返、暫存還原、專案 ZIP 往返、PNG 與批次 ZIP 實際下載；12 組直接改字、取消與原圖還原；選項完整套圖與重複選項文字、評分窄欄編輯，以及說明卡 3 種視窗的最大縮放。縮圖品牌框另以真實 PNG 驗證專案包圖片回填。固定基準 `98d3b0da14d9bc5224b8a3d714ef11279f72e5f6` 為 PR #69 已通過驗收的上線版，用於本輪 Canvas 像素比對；不可默默更新為候選版。
