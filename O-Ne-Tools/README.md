...
共用專案包與檔名由 `project-package-v1.js` V1.3.2 提供。12 個共用備份工具由 `edit-backup-v1.js` 自動接入；常駐卡保留既有暫存資料格式，但介面也併入同一個完成後區塊。
...
- 13 個可用工具共用 `batch-render-v1.js` V1.3.1，並放在同一個完成後區塊；每頁只會建立一個完成後區塊。
...
- 13 個正式可用工具共用 `ai-json-guide-v1.js` V1.3.1；每張卡的「給 AI 的 JSON 格式」收在最下方完成後區塊，JSON 範例預設折疊，不再壓縮主要編輯空間。說明卡以完整專案 ZIP 搬移圖片，並維持舊 `.onecard` 載入相容。
