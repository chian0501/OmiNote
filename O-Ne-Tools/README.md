...
- 「內容設定」可暫時收起整個側欄；較長的設定群組可個別展開。版型／狀態選單集中在左側「內容與設定」頂部。評分卡的項目名稱獨占一行，顯示方式與分數在下一行，文字結果使用完整寬度。
- 點字卡上的可編輯文字可直接改字，與左側原生欄位同步；支援中文輸入法、完成與取消。單行按 Enter 完成，多行按 Ctrl／Cmd + Enter 完成，Esc 取消。
- 焦點內容卡使用卡面原位編輯與浮動文字工具列：標題、內文、項目和步驟可直接輸入，調整標題／內文共用字級、字色，或選字套用既有【強調】語法。取消會還原文字與格式；常用標籤加入 COST，標題前方標籤對齊第一行文字中心。
- 評分卡與片尾結算卡新建／重設時預設透明背景；載入 JSON、專案 ZIP 或手動暫存時，仍依該專案保存的背景設定還原。
- 評分卡可點預覽的左／右圖片直接換圖，鍵盤 Tab＋Enter 也可操作；圖片各自保留比例完整呈現，換圖會一併存進原有完整專案包。標籤與店家標題、評分項目、價格與心得預設收合，影像設定預設展開。
- 評分卡另有「左側上下雙圖／右側上下雙圖」：上圖、下圖沿用兩個獨立來源，等高版位以 contain 完整呈現，間隔 24px；單張隱藏時另一張使用整欄，切換版型保留來源。JSON 保存 stack-left／stack-right 和 vertical 設定，ZIP 含兩張原圖；舊 left／right／both／none 專案仍可載入。
- 對話卡的左右角色設定並排顯示，交換按鈕位於上方；短字卡使用 220px 高預覽區，桌面加寬設定欄，窄螢幕改上下配置。
- 全部預覽最多 100% 原尺寸；預設「適合畫面」只縮小，不放大。焦點內容卡也使用相同縮放規則，保留元件／影片預覽切換；說明卡的既有直接編輯預覽同樣限制最大 100%。
- 選項卡的主要按鈕「輸出整組 PNG」一次下載 ZIP：全部暗 1 張，加上每個選項各自亮 1 張。5 個選項共 6 張，輸出期間保留目前畫面與編輯狀態；「僅下載目前畫面 PNG」仍可單張輸出。
- `card-workspace-v1.js` 與 `card-workspace-v1.css` 整理編輯器介面；`card-direct-edit-v1.js` 追蹤原生文字及圖片座標並操作既有欄位。Canvas 正式樣式、輸出像素尺寸、JSON schema、本機暫存鍵值與原生事件處理保持相容。
...
- 焦點卡與評分卡分開選擇「圖片在左／右」及「單張／上下兩張／並排兩張」，也保留左右各一張。切換排列保留兩張原圖與各自裁切。
- 縮圖旁按「編輯裁切」開啟共用圖片視窗，支援完整圖、自由裁切、1:1／16:9／3:4，拖曳選取框、邊角、方向鍵或百分比欄位；取消不更動卡面。PNG 套用裁切，ZIP 保存原圖與裁切設定。
- 焦點卡設定整理為圖片、文字、標籤、版面字級、附加文字；後三組預設收合。開啟標籤時側邊色條跟隨標籤底色。
- 評分卡點卡面文字可直接編輯及調字級，個別評分列各自保存；背景設定移至最下方並預設收合，背景維持預設透明。
- 既有 JSON schema 不變。評分新增可選 text_styles、各評分列 text_styles、圖片 crop；缺少新欄位時使用原字級與完整圖片。
- 新增 3 組 Chromium 驗收包含同側雙圖、獨立裁切、鍵盤／拖曳、取消還原、原圖 ZIP、字級設定、色條同步及窄螢幕視窗；仍須查看實際截圖後才能合併。
