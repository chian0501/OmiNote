...
(async () => {
  ...
  assert(backupSource.includes('project-package-v1.js?v=1104'), 'shared backup must synchronously bridge to the cache-busted project package helper');
  ...
  for (const file of sharedEditors) {
    ...
    assert(html.includes(file === 'focus-card.html' ? 'edit-backup-v1.js?v=1214' : 'edit-backup-v1.js?v=121'), file + ' must still load the shared backup bridge');
  }
