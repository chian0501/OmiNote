...
assert(source.includes('drawAsset(s,X.left'), 'renderer must pass the full left image crop state');
assert(source.includes('drawAsset(s,X.right'), 'renderer must pass the full right image crop state');
...
const savedImages = sequence.serializeImages({ placement: 'grid-left', scale: 40, left: freeCropAsset, right: cropAsset, third: freeCropAsset, fourth: cropAsset }, layout.focusCrop);
assert.strictEqual(savedImages.left.cropWidth, 50, 'history captures first image crop');
assert.strictEqual(savedImages.third.cropWidth, 50, 'history captures third image crop');
assert.strictEqual(savedImages.fourth.fit, 'cover', 'history captures fourth image crop');
assert(source.includes('serializeImages(fl,focusCrop)'), 'JSON and history serialize all slots');
...
assert(source.includes('V0.6.0_20260907'));
