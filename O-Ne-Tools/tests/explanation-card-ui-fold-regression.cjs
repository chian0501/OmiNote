...
assert(html.includes('explanation-card-ui-fold-v1.js?v=3'),'fold JS must load with the current cache key');
assert(html.indexOf('explanation-card-v049.js?v=0491')<html.indexOf('explanation-card-ui-fold-v1.js?v=3'),'fold enhancer must run after existing V0.4.9 runtime');
...
assert(js.includes("TEMPLATES.steps.make=()=>["),'step template must use the compact one-line variant');
...
assert(js.includes("block('body','<ul><li>第一個真正重要的重點</li></ul>')"),'first list item must be its own bullet block');
assert(js.includes("block('body','<ul><li>第二個觀眾會想知道的資訊</li></ul>')"),'second list item must be its own bullet block');
assert(js.includes("block('body','<ul><li>第三個結論或建議</li></ul>')"),'third list item must be its own bullet block');
