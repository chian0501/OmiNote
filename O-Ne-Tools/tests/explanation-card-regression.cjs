...
assert(html.includes('<title>O-Ne 說明卡生成器 V0.4.0 CANDIDATE</title>'), 'candidate version must be visible');
assert(html.includes('edit-backup-v1.js?v=1217'), 'shared manual backup library must load');
assert(html.includes('explanation-card-v040.css?v=040'), 'gallery CSS must load');
assert(html.includes('explanation-card-v040.js?v=040'), 'gallery runtime must load');
...
for (const id of ['single', 'split', 'hero-right', 'hero-bottom', 'grid']) {
...
assert(gallerySource.includes("schema:'o-ne.explanation-card.candidate.v0.4.0'"), 'candidate JSON schema must be versioned');
...
const expectedCounts = { single: 1, split: 2, 'hero-right': 3, 'hero-bottom': 3, grid: 4 };
...
console.log('PASS: explanation-card V0.4.0 candidate keeps Word-lite and adds one-to-four-image gallery layouts with portable project assets.');
