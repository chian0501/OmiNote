...
(function (global) {
  ...
  function storageKey(toolId) {
    return 'one.edit-history.v1:' + toolId;
  }
  ...
  function fieldElements(root) {
    return Array.prototype.slice.call((root || document).querySelectorAll('input[id],select[id],textarea[id]'))
      .filter(function (element) {
        return element.type !== 'file' && !element.closest('[data-one-backup-ui]');
      });
  }
  ...
  function collectFields(root) {
    ...
    fieldElements(root).forEach(function (element) {
      if ((element.type === 'radio' || element.type === 'checkbox')) {
        fields[element.id] = { kind: 'checked', value: Boolean(element.checked) };
      } else {
        fields[element.id] = { kind: 'value', value: element.value };
      }
    });
    return fields;
  }
  ...
  function defaultCapture(config) {
    return {
      fields: collectFields(config.root || document),
  ...
  function capture(instance) {
    return clone(instance.config.capture
      ? instance.config.capture()
  ...
  function restoreSelected(instance) {
    ...
    try {
      ...
      setStatus(instance, '還原失敗：' + error.message, true);
    }
  }
  ...
  function importFile(instance, file) {
    ...
    if (file.size > MAX_JSON_BYTES) {
      setStatus(instance, '載入失敗：JSON 不可超過 1 MB。', true);
      return;
    }
  ...
  function attach(instance) {
    ...
    instance.clearButton.onclick = function () {
      try {
        ...
        setStatus(instance, '清除失敗：' + error.message, true);
      }
    };
